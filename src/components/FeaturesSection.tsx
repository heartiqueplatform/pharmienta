...
export const FeaturesSection: React.FC<FeaturesSectionProps> = ({ onOpenTrial, isDarkMode = false }) => {
  ...
  return (
    <section id="features" className={`py-20 md:py-28 relative transition-colors duration-200 ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'
      }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {FEATURES.map((feature) => (
            <div
              key={feature.id}
              className={`group rounded-3xl p-6 flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 shadow-md hover:shadow-xl ${isDarkMode
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between mb-5">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform ${isDarkMode ? 'bg-[#21262d]' : 'bg-white shadow-xs'
                    }`}>
                ...
                {/* Title & Tagline */}
                <h3 className={`text-lg font-bold font-heading mb-1.5 transition-colors ${isDarkMode ? 'text-white group-hover:text-blue-300' : 'text-slate-900 group-hover:text-[#003366]'
                  }`}>
                  {feature.title}
                </h3>
              ...
              {feature.highlight && (
                <div className={`pt-3 text-[11px] flex items-start gap-1.5 ${isDarkMode ? 'border-t border-slate-800 text-slate-400' : 'border-t border-slate-200/80 text-slate-600'
                  }`}>
        ...
        <div className="text-center">
          <button
            ...
            onClick={onOpenTrial}
            className={`inline-flex items-center gap-2 text-sm font-bold px-6 py-3 rounded-xl transition-all shadow-md cursor-pointer ${isDarkMode
