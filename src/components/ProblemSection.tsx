...
export const ProblemSection: React.FC<ProblemSectionProps> = ({ onOpenTrial, isDarkMode = false }) => {
  ...
  return (
    <section id="problem" className={`py-20 md:py-28 transition-colors duration-200 ${
      isDarkMode ? 'bg-[#0d1117]' : 'bg-slate-100/70'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-300 text-xs font-bold mb-4 shadow-xs">
            <AlertTriangle className="w-3.5 h-3.5" />
            <span>The Reality for Kenyan Chemists</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 mb-16">
          {PAIN_POINTS.map((item, index) => (
            <div
              key={item.id}
              className={`group rounded-3xl p-6 sm:p-8 transition-all duration-300 shadow-md hover:shadow-xl ${
                isDarkMode 
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-3 flex-1">
                  ...
                  <div className={`rounded-xl p-3.5 text-xs font-medium ${
                    isDarkMode ? 'bg-red-950/40 text-red-300' : 'bg-red-50 text-red-800'
                  }`}>
                    <strong className="font-bold">Cost to You:</strong> {item.consequence}
                  </div>

                  <div className={`pt-2 flex items-start gap-2 text-xs font-medium ${
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-xl bg-gradient-to-r from-[#003366] via-[#004080] to-[#B30000] p-1">
          <div className={`rounded-[23px] p-8 sm:p-10 lg:p-12 text-center max-w-4xl mx-auto backdrop-blur-xl ${
            ...
            <h3 className={`text-2xl sm:text-3xl lg:text-4xl font-extrabold font-heading mb-4 ${
              isDarkMode ? 'text-white' : 'text-slate-900'
            }`}>
            ...
            <p className={`text-base sm:text-lg mb-8 max-w-2xl mx-auto leading-relaxed ${
              isDarkMode ? 'text-slate-300' : 'text-slate-600'
            }`}>
