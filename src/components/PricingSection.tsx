...
export const PricingSection: React.FC<PricingSectionProps> = ({ onOpenTrial, isDarkMode = false }) => {
  ...
  const [activeTab, setActiveTab] = useState<'pharmacy' | 'supplier'>('pharmacy');
  ...
  // Supplier pricing
  const supplierMonthlyPrice = 1999;
  const supplierYearlyPrice = 19990;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-extrabold font-heading tracking-tight mb-5 ${isDarkMode ? 'text-white' : 'text-slate-900'
            ...
            Simple, Transparent Pricing for the{' '}
            <span className="text-[#2ea043]">Pharmienta Ecosystem</span>
          </h2>

          <p className={`text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-300' : 'text-slate-600'
            }`}>
            Choose the plan that fits your pharmacy or supplier business. No hidden fees. Cancel anytime.
          </p>
        </div>

        {/* Platform Tabs */}
        <div className="flex justify-center mb-12">
          <div className={`inline-flex p-1 rounded-2xl ${isDarkMode ? 'bg-[#21262d]' : 'bg-slate-100'
            }`}>
            <button
              onClick={() => setActiveTab('pharmacy')}
              className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all cursor-pointer flex items-center gap-2 ${activeTab === 'pharmacy'
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all cursor-pointer flex items-center gap-2 ${activeTab === 'supplier'
                ? 'bg-[#003366] text-white shadow-sm'
                : isDarkMode ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'
                }`}
            >
        ...
        {activeTab === 'supplier' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch max-w-5xl mx-auto mb-16">
            {/* Supplier Main Card */}
            <div className={`lg:col-span-7 -mx-4 sm:mx-0 w-[calc(100%+2rem)] sm:w-full rounded-none sm:rounded-3xl p-6 sm:p-10 shadow-2xl relative flex flex-col justify-between ${isDarkMode ? 'bg-[#161b22]' : 'bg-gradient-to-br from-[#003366] to-[#1a4a7a] text-white'
              }`}>
              <div>
                <div className="mb-4">
                  <h3 className="text-2xl font-bold font-heading text-white flex items-center gap-2">
                  ...
                  <p className="text-xs text-blue-200 dark:text-slate-400 mt-0.5">
                    Complete B2B platform for pharmaceutical suppliers and distributors
                  </p>
                </div>
                ...
                <div className={`my-6 p-6 rounded-2xl ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white/10 backdrop-blur-md'
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-5xl sm:text-6xl font-black font-heading text-white">
                      KES {billingCycle === 'yearly' ? supplierYearlyPrice.toLocaleString() : supplierMonthlyPrice.toLocaleString()}
                    </span>
                    <span className="text-blue-200 dark:text-slate-400 text-sm font-bold">
                  ...
                  {billingCycle === 'yearly' && (
                    <p className="text-xs text-emerald-300 font-bold mt-2 flex items-center gap-1.5">
                      <Check className="w-4 h-4 text-[#2ea043]" /> Save 17% with annual billing
                    </p>
                  )}
                </div>

                {/* Billing Toggle for Supplier */}
                <div className={`inline-flex p-1 rounded-2xl w-full mb-6 ${isDarkMode ? 'bg-[#21262d]' : 'bg-white/15'
                  }`}>
                  <button
                    onClick={() => setBillingCycle('monthly')}
                    className={`w-1/2 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer ${billingCycle === 'monthly'
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs mb-8">
                  {[
                    ...
                    'Receive & Manage Pharmacy Orders',
              ...
              >
                ...
                <span>Start Supplier Onboarding</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </div>

            {/* Supplier Sidebar - Why Choose Supplier */}
            <div className={`lg:col-span-5 -mx-4 sm:mx-0 w-[calc(100%+2rem)] sm:w-full rounded-none sm:rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-xl ${isDarkMode ? 'bg-[#161b22]' : 'bg-slate-50'
              }`}>
              <div>
                <h3 className={`text-xl font-bold font-heading mb-1 ${isDarkMode ? 'text-white' : 'text-slate-900'
                ...
                <p className="text-xs text-slate-400 mb-6">
                  Connect with pharmacies and grow your distribution business.
                </p>

                <div className="space-y-4">
                  <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'}`}>
                    ...
                    <p className="text-xs text-slate-400">
                      Connect with Pharmienta pharmacies looking for reliable suppliers.
                    </p>
                  </div>

                  <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <Zap className="w-4 h-4 text-[#2ea043]" />
                      <h4 className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                        Faster Order Processing
                      </h4>
                    </div>
                    <p className="text-xs text-slate-400">
                      Receive and process orders digitally instead of manual phone calls and WhatsApp.
                    </p>
                  </div>
                  ...
                  <div className={`p-4 rounded-2xl ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'}`}>
                    ...
                    <p className="text-xs text-slate-400">
                      Know exactly what you have, what's reserved, and what's available.
                    </p>
                  </div>
                </div>
              </div>
        ...
        {/* Interactive Pharmacy ROI Calculator Widget - Shown only for Pharmacy tab */}
        {activeTab === 'pharmacy' && (
        ...
        {/* Supplier ROI Calculator - Shown only for Supplier tab */}
        {activeTab === 'supplier' && (
          <div className={`max-w-4xl mx-auto rounded-3xl p-6 sm:p-8 shadow-xl ${isDarkMode ? 'bg-[#161b22]' : 'bg-slate-50'
            }`}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h4 className={`text-lg font-bold font-heading ${isDarkMode ? 'text-white' : 'text-slate-900'
                  ...
                  Grow Your Supplier Business
                </h4>
                <p className="text-xs text-slate-400">
                  Connect with pharmacies and streamline your order fulfillment process.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className={`p-4 rounded-2xl shadow-xs ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'
                ...
                <p className="text-[11px] font-semibold text-slate-400">Pharmacies Reached</p>
                <p className={`text-lg font-bold font-mono mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'
                  }`}>
                  200+<span className="text-xs font-normal text-slate-400"> potential</span>
                </p>
              </div>

              <div className={`p-4 rounded-2xl shadow-xs ${isDarkMode ? 'bg-[#0d1117]' : 'bg-white'
                }`}>
                <p className="text-[11px] font-semibold text-slate-400">Order Processing Time</p>
                <p className={`text-lg font-bold font-mono mt-1 ${isDarkMode ? 'text-white' : 'text-slate-900'
                  }`}>
                  -70%<span className="text-xs font-normal text-slate-400"> faster</span>
                </p>
              </div>

              <div className="p-4 rounded-2xl shadow-xs bg-emerald-100 text-emerald-900 dark:bg-emerald-950/60 dark:text-emerald-200">
                <p className="text-[11px] font-bold">Error Reduction</p>
                <p className="text-xl font-black text-[#2ea043] font-mono mt-1">
                  -85%<span className="text-xs font-semibold"> in order errors</span>
                </p>
              </div>
            </div>
          </div>
