...
export const BenefitsSection: React.FC<BenefitsSectionProps> = ({ onOpenTrial, isDarkMode = false }) => {
  ...
  return (
    <section id="benefits" className={`py-20 md:py-28 relative transition-colors duration-200 ${isDarkMode ? 'bg-[#0d1117]' : 'bg-slate-100/60'
      }`}>
            ...
            return (
              ...
              >
                ...
                <div className="w-full lg:w-1/2">
                  <div className={`relative -mx-4 sm:mx-0 w-[calc(100%+2rem)] sm:w-full rounded-none sm:rounded-3xl p-6 sm:p-10 shadow-xl overflow-hidden group ${isDarkMode ? 'bg-[#161b22]' : 'bg-white'
                    }`}>
                    {/* Background subtle glow circle */}
                    <div className={`absolute -top-10 -right-10 w-48 h-48 rounded-full blur-3xl group-hover:scale-125 transition-transform ${isDarkMode ? 'bg-[#003366]/40' : 'bg-blue-100/60'
                      }`} />

                    <div className="relative z-10 flex flex-col justify-between h-full">
                      <div className="flex items-center justify-between mb-8">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-md ${isDarkMode ? 'bg-[#21262d]' : 'bg-slate-50'
                          }`}>
                          {getIcon(benefit.iconName)}
                        </div>
                        <span className={`px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'bg-[#21262d] text-slate-300' : 'bg-slate-100 text-slate-700'
                          }`}>
                          {benefit.badge}
                        </span>
                      </div>
                      ...
                      <div className="space-y-2 mb-6">
                        <span className={`text-5xl sm:text-6xl lg:text-7xl font-black font-heading tracking-tight ${isDarkMode ? 'text-white' : 'text-[#003366]'
                          }`}>
                          {benefit.metric}
                        </span>
                      ...
                      <div className={`p-3.5 rounded-xl flex items-center justify-between text-xs font-medium ${isDarkMode ? 'bg-[#0d1117] text-slate-400' : 'bg-slate-50 text-slate-600'
                        }`}>
                ...
                <div className="w-full lg:w-1/2 space-y-6">
                  ...
                  <h3 className={`text-2xl sm:text-3xl font-bold font-heading leading-snug ${isDarkMode ? 'text-white' : 'text-slate-900'
                    }`}>
                    {benefit.title}
                  </h3>
                  ...
                  <ul className="space-y-3 pt-2">
                    {benefit.detailPoints.map((point, pIdx) => (
                      <li key={pIdx} className={`flex items-start gap-3 text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-700'
                        }`}>
